...
# Submit redis 
JOB_NAME=$(sbatch --time=${TIME} --job-name=redis_${PORT} --partition=${PARTITION} --account=parfitmulticell submit_redis.sh ${PWD} ${PORT})
...
sbatch --time=${TIME} --job-name=python_${PORT} --account=parfitmulticell --partition=${PARTITION} submit_python.sh ${PWD} ${REDIS_IP} ${PORT} ${PYTHONFILE}
...
do 
	...
	sbatch --time=${TIME} --job-name=${NAME} --account=parfitmulticell --cpus-per-task=${CPUSPERTASK} --partition=${PARTITION} submit_worker.sh ${REDIS_IP} ${PORT} ${TIME} ${CPUSPERTASK} ${PWD}
done
